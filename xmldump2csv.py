...
def create_stations():
    ...
    with open(os.path.join(CSV_DIRECTORY, 'stations.csv'), 'w') as f:
        ...
        headernames = {'id': 'id:ID(Station)'}
        writer = csv.DictWriter(f, [headernames.get(x, x) for x in fieldnames])
        stations = map(lambda x: {headernames.get(key, key): x[key] for key in fieldnames}, track_a_bike.stations.values())
        writer.writeheader()
...
def create_bikes():
    ...
    with open(os.path.join(CSV_DIRECTORY, 'bikes.csv'), 'w') as f:
        ...
        bikes_list.sort(key=lambda x: x['id:ID(Bike)'])
